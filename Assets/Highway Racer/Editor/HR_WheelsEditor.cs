...
public class HR_WheelsEditor : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        for (int i = 0; i < prop.wheels.Length; i++) {
            ...
            prop.wheels[i].unlocked = EditorGUILayout.ToggleLeft("Unlocked", prop.wheels[i].unlocked, GUILayout.Width(150f)); prop.wheels[i].price = EditorGUILayout.IntField("Price", prop.wheels[i].price, GUILayout.Width(150f)); GUI.color = Color.red; if (GUILayout.Button("Remove")) { RemoveWheel(i); }
            GUI.color = orgColor;
