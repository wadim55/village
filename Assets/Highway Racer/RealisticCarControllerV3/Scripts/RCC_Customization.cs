...
public class RCC_Customization : MonoBehaviour {
    ...
    public static void ChangeWheels(RCC_CarControllerV3 vehicle, GameObject wheel, bool applyRadius) {
        ...
        for (int i = 0; i < vehicle.allWheelColliders.Length; i++) {
            ...
            GameObject newWheel = (GameObject)Instantiate(wheel, vehicle.allWheelColliders[i].wheelModel.position, vehicle.allWheelColliders[i].wheelModel.rotation);
            newWheel.transform.SetParent(vehicle.allWheelColliders[i].wheelModel, true);

            if (vehicle.allWheelColliders[i].wheelModel.localPosition.x > 0)
                newWheel.transform.position += vehicle.allWheelColliders[i].transform.right * .075f;
            else
                newWheel.transform.position -= vehicle.allWheelColliders[i].transform.right * .075f;
